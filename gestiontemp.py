...
class myapp:
    ...
    def init_lpanel(self):
        ...
        crow += 1
        # Check button FREEZE
        tk.Checkbutton(self.lpanel, text='FREEZE', variable=self.cbvar, onvalue=1, offvalue=0,
                       command=self.cb_selection).grid(column=0, row=crow)
        crow += 1
        # ---
        tk.Label(self.lpanel, text="---").grid(column=0, row=crow)
        crow += 1
        ...
        self.btn_StartStop = Button(self.lpanel, textvariable=self.btn_StartStop_txt, command=self.startstopit,
                                    bg="black", fg="white").grid(column=1, row=crow)
        crow += 1
        # ---
        tk.Label(self.lpanel, text="---").grid(column=0, row=crow)
        crow += 1
        ...
        self.slidebar = tk.Scale(self.lpanel, variable=self.str_Thermostat, from_=30, to=0, resolution=0.5, length=200,
                                 tickinterval=5, orient=VERTICAL, command=self.cbk_thermostat).grid(column=0, row=crow)
        crow += 1
        # exit button
        tk.Label(self.lpanel, text="Press to:").grid(column=0, row=crow)
    ...
    def cbk_refresh(self):  # Called each time
        ...
        print("Read ", self.datafilename, " with Tint:", Tint, "°C and Text:", Text, "°C at time ",
              strftime("%H:%M:%S", gmtime(self.ctime * REAL_DELAY_VALUE)))
    ...
    def cbk_thermostat(self, value):  # Called when slider has been modified
        print("Write ", self.consignefilename, " with ", self.str_Thermostat.get(), "°C at time ",
              strftime("%H:%M:%S", gmtime(self.ctime * REAL_DELAY_VALUE)))
        # Check lock file and add previous value for the plot if it exists
        try:
            fp = open(self.lockfilenameconsigne, "x")
        except IOError:
            print(self.consignefilename, " is locked (by ", self.lockfilenameconsigne, "). ", self.consignefilename,
                  " is not written.")
        else:
            # Lock consigne
            fp.close()
